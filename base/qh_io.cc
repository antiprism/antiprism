...
void qh_produce_output(void) {
  ...
  if (qh PRINTstatistics) {
    ...
    size in bytes: merge %lu ridge %lu vertex %lu facet %lu\n\
	    ...
	    (unsigned long)sizeof(mergeT), (unsigned long)sizeof(ridgeT),
	    (unsigned long)sizeof(vertexT), (unsigned long)sizeof(facetT),
	    qh normal_size, d_1, (unsigned long)(d_1 + SETelemsize));
  }
